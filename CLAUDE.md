...
│   │   ├── components/
│   │   │   ├── ViewToggle.jsx        # Switch between Dashboard ↔ Map ↔ Table views
...
### View Toggle UX
- Header bar, right side. Segmented control: `[📊 Dashboard]  [🗺️ Map]  [📋 Table]`
- All views share filter state. URL hash: `#dashboard` / `#map` / `#table` (bookmarkable). Default: `#dashboard`.
- Filter panel renders at bottom on dashboard view, at top on map and table views.
...
- URL hash routing: `#dashboard` / `#map` / `#table`. Default: `#dashboard`.
