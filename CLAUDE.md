...
│   │   ├── components/
│   │   │   ├── ViewToggle.jsx        # Switch between Dashboard ↔ Map ↔ Table ↔ Industries views
│   │   │   ├── DashboardView.jsx     # Charts + summary cards layout
│   │   │   ├── TableView.jsx         # Full-featured sortable/filterable table
│   │   │   ├── MapView.jsx           # Choropleth world map view
│   │   │   ├── MapCountryDetail.jsx  # Side panel: actions for selected country
│   │   │   ├── MapLegend.jsx         # Gradient legend for map coloring
│   │   │   ├── IndustryView.jsx      # Industry sector view: stats, charts, sector cards
│   │   │   ├── IndustryComparisonChart.jsx  # Bar chart: action counts by sector
│   │   │   ├── IndustryRateChart.jsx        # Bar chart: max tariff rate by sector
│   │   │   ├── IndustryCard.jsx      # Expandable sector card with verified/external data
...
│   │   │   ├── FilterPanel.jsx       # Collapsible filters: country dropdown, type, status, date
...
│   │       └── countries-110m.json   # Natural Earth TopoJSON boundaries
│   ├── public/
...
#### 🗺️ Map View
Layout:
1. Same MVP Banner, Header, and Filter Panel (top).
2. **Choropleth World Map** — countries colored by specific action count (gray → blue → navy). Equal Earth projection.
3. Hover tooltip: country name, targeted count, global count.
4. Click country → **MapCountryDetail** side panel (desktop) or below map (mobile). Shows grouped Targeted/Global actions.
5. Click action row → opens ActionDetailModal.
6. **MapLegend** — gradient bar with 0–max labels.
7. Global actions badge: shows count of "All countries" actions.
...
Layout (top to bottom):
1. Same MVP Banner, Header, and Filter Panel (top).
2. **Stats Row** (3 cards): Industries Affected, Largest Sector, Sector-Actions (incl. multi-sector).
3. **Charts Row** (2 charts side by side on lg): Left: Action counts by sector. Right: Max tariff rate by sector.
4. **Sector Cards Grid** (2-col on lg) — expandable IndustryCards per sector.
5. **Source Disclaimer** — footnote distinguishing verified (CSMS) from external data.

**Sector cards** expand to show:
- **Verified data** (blue border): claims with CSMS # citations from cached bulletin texts.
- **External estimates** (amber border): policy analysis data with source attribution.
- **Action list** (sorted by date desc, clickable → ActionDetailModal).

**Industry mapping** (`industryMap.js`): 8 sectors (Primary Metals, Automotive, Semiconductors, Agriculture, Energy, Consumer Goods, Luxury Goods, Compliance). Each of the 41 trade actions maps to one or more sectors. Uses ID-based mapping with keyword fallback for future scraper-generated actions.

**Data tiers:**
- Tier 1 (Verified): from cached CSMS bulletin texts (`scraper/cache/texts/csms_{id}.txt`), with `csms_id` + `excerpt`.
- Tier 2 (External): from public policy analysis, with `source` attribution.

### View Toggle UX
- Header bar, right side. Segmented control: `[📊 Dashboard]  [🗺️ Map]  [📋 Table]  [🏭 Industries]`
- All views share filter state. URL hash: `#dashboard` / `#map` / `#table` / `#industries` (bookmarkable). Default: `#dashboard`.
- Filter panel renders at bottom on dashboard view, at top on map, table, and industries views.
...
- Implement **Dashboard ↔ Map ↔ Table view toggle** with shared filter state and URL hash routing.
...
- All views read from same `useTradeData` hook.
- Shared filter state in parent component or context.
- URL hash routing: `#dashboard` / `#map` / `#table` / `#industries`. Default: `#dashboard`.
- Same `<FilterPanel />` and `<ActionDetailModal />` components.
- Industries view uses `useIndustryData` hook which processes `filteredActions` from `useTradeData`.
